...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	// PROFILE PIC SECTION
	$lch_PictureData = base64_decode(str_pad(strtr($_POST["profile-pic"]["data"], '-_', '+/'), strlen($_POST["profile-pic"]["data"]) % 4, '=', STR_PAD_RIGHT));
	$lch_PictureData = mysql_real_escape_string ($lch_PictureData);

	//echo $lch_PictureData ." ----- ";

	$lch_PictureName = mysql_real_escape_string ($_POST["profile-pic"]["name"]);
	$lch_PictureType = mysql_real_escape_string ($_POST["profile-pic"]["type"]);
	...
	if (!$SQL_RESULT){
		...
		$lch_Query = "UPDATE mstuser SET mstuser.user_image_code='$lin_imagecode',updated_at=NOW(),updated_user_mst_code='$lch_UserMSTCode' WHERE mstuser.code='$lch_InputUserMSTCode' ";
		$SQL_RESULT=mysql_query($lch_Query) or die(mysql_error());
	...
	if ($llo_UpdAddSomeRecords){
		$larr_ResultQueryArray = array(
			...
			"new_image_code" => $lin_imagecode,
			"input_user_code" => $lch_InputUserMSTCode
		);
	} // if ($llo_UpdAddSomeRecords){
	else {
		$larr_ResultQueryArray = array(
