		...
		if (isset($_POST['action']) && $_POST['action']==='get_table_record_count') {
			...
			*/
			...
			foreach ($sanitizedWhereInConditions as $keyName => $keyValue) {
				...
				if (!empty($arr)){
					$inConditions = $inConditions . "$keyName IN (";
					foreach ($arr as $str){
