...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {	
	...
	$larr_Legends = array("Earnings For the month");
	...
	$lch_Query = $lch_Query . " AND mstmoneytrailtype.short_name NOT LIKE 'AR%' ";
	...
	$lch_Query = "SELECT SUM(trnmoneytrail.total_amount) AS 'total_amount', trnmoneytrail.account_money_trail_type_mst_code, MONTH(trnmoneytrail.money_trail_date) as 'created_month'   ";
	$lch_Query = $lch_Query . " FROM trnmoneytrail ";
	...
	$lch_Query = $lch_Query . " AND mstmoneytrailtype.is_payable_debt_account != '1' ";
	$lch_Query = $lch_Query . " AND (mstmoneytrailtype.short_name != 'ARI' AND mstmoneytrailtype.short_name != 'API') ";
	$lch_Query = $lch_Query . " GROUP BY MONTH(trnmoneytrail.money_trail_date), trnmoneytrail.account_money_trail_type_mst_code ";
	$lch_Query = $lch_Query . " ORDER BY MONTH(trnmoneytrail.money_trail_date), trnmoneytrail.account_money_trail_type_mst_code ASC ";
	$SQL_RESULT_TRNMONEYTRAIL=mysql_query($lch_Query) or die(mysql_error());
	while($larr_ResultRow_TRNMONEYTRAIL=mysql_fetch_array($SQL_RESULT_TRNMONEYTRAIL)) {
		//$larr_outputArray[$larr_ResultRow_TRNMONEYTRAIL["account_money_trail_type_mst_code"]]["running_balance"] += (floatval($larr_ResultRow_TRNMONEYTRAIL["total_amount"]) * -1);
		...
		$larr_MoneyTrail[$larr_ResultRow_TRNMONEYTRAIL["account_money_trail_type_mst_code"]][intval($larr_ResultRow_TRNMONEYTRAIL["created_month"])] -= (floatval($larr_ResultRow_TRNMONEYTRAIL["total_amount"]));
