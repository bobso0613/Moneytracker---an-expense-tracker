...
//                        echo 'END OF TABLE ' , $this->tableLevel , ' CELL<br />';
                        $sheet->getStyle($column . $row)->getFont()->setSize(6);
                        ...
                        foreach ($attributeArray as $attributeName => $attributeValue) {
                            switch ($attributeName) {
                                case 'style':
                                    $explodeSemiColons = explode(';', $attributeValue);
                                    foreach($explodeSemiColons as $explodeSemiColon) {
                                        $explodeColon = explode(':', $explodeSemiColon);
                                        switch (trim(@$explodeColon[0])) {
                                            case 'font-weight':
                                                switch (trim(@$explodeColon[1])) {
                                                    case 'bold':
                                            ...
                                            case 'text-align':
                                                switch (trim(@$explodeColon[1])) {
                                                    ...
                                                    break;
                                                    case 'right':
                                                        $sheet->getStyle($column . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
                                                    break;
                                                }
                                            break;
                                        }
